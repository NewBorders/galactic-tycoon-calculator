...
- Adjusted weighted queue calculations so production weights only influence time allocation, preserving full throughput while letting recipes balance inputs (e.g. cows vs. meat).
- Replaced production weights with queue time share overrides that normalize to 100%, updating the production engine, store, UI, and localisation accordingly.
